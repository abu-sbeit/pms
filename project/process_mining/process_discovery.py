...
from pm4py.statistics.variants.log.get import get_variants, get_variants_sorted_by_count, get_variants_along_with_case_durations
...
class ProcessDiscovery:
  ...
  def apply(log: EventLog, admitted_variants: List[List[str]]) -> EventLog: 
    parameters = {}
...
class GnnMiner(ProcessDiscovery):
  ...
  def discover(self, export='', conformance_check=True, beam_width=1, beam_length=1, number_of_petrinets=1,  topXTraces=None,
    ...
    if conformance_check:
      ...
      log_top_x = self.apply(log, variants)
    ...
    if conformance_check  and len(results) > 1:
...
class InductiveMiner(ProcessDiscovery):
  ...
  def discover_prom(self, export_fn):
    ...
    parameters = {
      'prom_directory': Path('/Users/abdalrhman/Downloads/new/prom-lite-1.3-all-platforms'),  # Directory to the ProM installation.
...
class SplitMiner(ProcessDiscovery):
  def __init__(self, fLogFilename):
    super().__init__(fLogFilename)
    self.log_filename = fLogFilename
    self.split_miner_dir = '/Users/abdalrhman/Downloads/split-miner-2'

...
class Entropia:
  def __init__(self, log_filename, pnml_filename):
    self.basedir = '/Users/abdalrhman/Documents/apd-ml/jbpt/codebase/jbpt-pm/entropia'
...
def find_gcn_model(dataset, datadir2='evaluation_data'):
  datadir = f'/Users/abdalrhman/Documents/thesis_data/{datadir2}'
...
def eval_data(samples, methods, directory):
  for sample in tqdm.tqdm(samples):
    ...
    for method in methods:
      ...
      cc_filename = f'{directory}/{sample}/predictions/data_{method}_cc.json'
      ...
      export = True
      if export:
        with open(f'{cc_filename[:-5]}e.json', 'w') as f:
...
def test_data(samples, methods, directory):
  ...
  for sample in tqdm.tqdm(samples):
    ...
    for method in methods:
      ...
      cc_filename = f'{directory}/{sample}_{method}_cc.json'
      if not os.path.exists(cc_filename):
        print(f'{cc_filename} file not exists.')
        continue
...
if __name__ == '__main__':
  ...
  datasets = [
    ...
    ('BPI_2020_Permit_log', 75), #5
    ('BPI_2020_Prepaid_travel_cost', 30), #5
  ...
  models = [
    'simpleep144',
    'complexep100',
    'candidates_frequency_new_036'
  ]

  method = args.dataset
  mf_model = {'simple': 'gcn_simpleep144', 'complex': 'gcn_complexep100', 'medium': 'gcn_blahblah_repro'}[method]
  samples = datasets
  methods = [f'{mf_model}_{i}' for i in range(20)]

  eval_data(samples, methods, '/Users/abdalrhman/Documents/thesis_data/evaluation_data')

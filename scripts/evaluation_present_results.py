...
methods = {
  'gcn__all': 'Our approach', 'gcn_simple_all': 'Our approach (simple)', 'gcn_complex_all': 'Our approach (complex)',
...
methods_short = {
  'gcn__all': 'Ours', 'gcn_simple_all': 'low', 'gcn_complex_all': 'high',
...
def plot():
  ...
  for ax_i, metric in enumerate(metrics_to_plot):
    for index, approach in enumerate(approaches):
      ...
      if 'gcn_' in approach and len([x for x in approaches if 'gcn' in x]) > 1:
